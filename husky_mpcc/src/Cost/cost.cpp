...
ErrorInfo Cost::getErrorInfo(const ArcLengthSpline &track,const State &x) const
{
    ...
    // std::cout << contouring_error << std::endl;
    // std::cout << "xref: " << track_point.x_ref << ", yref: " << track_point.y_ref << std::endl;
...
CostMatrix Cost::getContouringCost(const ArcLengthSpline &track, const State &x,const int k) const
{
    ...
    Q_contouring_cost(si_index.w, si_index.w) = k < N ? cost_param_.q_w : cost_param_.q_w_N_mult * cost_param_.q_w;
...
CostMatrix Cost::getHeadingCost(const ArcLengthSpline &track, const State &x,int k) const
{
    ...
    theta_ref += 2.0*M_PI*std::round((x.th - theta_ref)/(2.0*M_PI));
    ...
    Q_heading_cost(si_index.th,si_index.th) = 2.0*cost_param_.q_mu;
    q_MPC q_heading_cost = q_MPC::Zero();
    q_heading_cost(si_index.th) = -2.0*cost_param_.q_mu*theta_ref;
...
CostMatrix Cost::getSoftConstraintCost() const
{
    ...
    // Z_cost(si_index.con_leftwheel,si_index.con_leftwheel) = cost_param_.sc_quad_tire;
    // Z_cost(si_index.con_rightwheel,si_index.con_rightwheel) = cost_param_.sc_quad_alpha;

    z_cost(si_index.con_track) = cost_param_.sc_lin_track;
    // z_cost(si_index.con_leftwheel) = cost_param_.sc_lin_tire;
    // z_cost(si_index.con_rightwheel) = cost_param_.sc_lin_alpha;
