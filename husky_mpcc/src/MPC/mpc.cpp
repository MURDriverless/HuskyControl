...
void MPC::setStage(const State &xk, const Input &uk, const State &xk1, const int time_step)
{
    ...
    else
    {
        ...
        stages_[time_step].ns = NS; // Slack Variables: track
    }
...
void MPC::unwrapInitialGuess()
{
    ...
    for(int i=1;i<=N;i++)
    {
        if((initial_guess_[i].xk.th - initial_guess_[i-1].xk.th) < -M_PI)
        {
            initial_guess_[i].xk.th += 2.*M_PI;
        }
        if((initial_guess_[i].xk.th - initial_guess_[i-1].xk.th) > M_PI)
        {
            initial_guess_[i].xk.th -= 2.*M_PI;
...
// Assume centerline path is optimal guess
void MPC::generateNewInitialGuess(const State &x0)
{
    ...
    for(int i = 1;i<=N;i++)
    {
        ...
        initial_guess_[i].xk.th = atan2(track_dpos_i(1),track_dpos_i(0));
        initial_guess_[i].xk.v = param_.initial_velocity;
        initial_guess_[i].xk.w = 0.0;
    }
    unwrapInitialGuess();
